...
const Index = ({
	...
	const totalRows = paginationMode === 'client' ? processedRows.length : totalCount ?? processedRows.length;
			...
			<div className="rtg__container">
							...
							{columns.map((col, idx) => {
								...
								return (
									<th
										...
										style={{
											'--col-min-width': col.width ? `${col.width}px` : 'auto',
										...
										<div className="rtg__th-inner">
											...
											{col.filterable && col.filterOptions && (
												<select
													...
													onChange={(e) => handleFilterApply(col.key, e.target.value)}
				...
				<div className="rtg__pager">
					<div className="rtg__pager-left">
						<div
							className="rtg__pager-info">Showing <strong>{Math.min((page - 1) * pageSize + 1, totalRows)}</strong> - <strong>{Math.min(page * pageSize, totalRows)}</strong> of <strong>{totalRows}</strong>
						</div>
					</div>
					
					<div className="rtg__pager-center">
						<button className="rtg__btn" onClick={() => changePage(1)} disabled={page <= 1}
						        aria-label="first page">⏮
						</button>
						<button className="rtg__btn" onClick={() => changePage(page - 1)} disabled={page <= 1}
						        aria-label="prev page">◀
						</button>
						<span className="rtg__page-indicator">Page</span>
						<span className="rtg__current-page" aria-live="polite">{page}</span>
						<span className="rtg__of">of</span>
						<span className="rtg__max-page">{maxPage}</span>
						<button className="rtg__btn" onClick={() => changePage(page + 1)} disabled={page >= maxPage}
						        aria-label="next page">▶
						</button>
						<button className="rtg__btn" onClick={() => changePage(maxPage)} disabled={page >= maxPage}
					...
					<div className="rtg__pager-right">
						...
						<select className="rtg__select" value={pageSize}
						...
						<div className="rtg__go-to">
							<input type="text" className="rtg__go-input" placeholder="Go to page" value={pageInput}
							       onChange={(e) => setPageInput(e.target.value)}/>
							<button className="rtg__go-btn" onClick={handlePageInputGo}>Go</button>
						</div>
					</div>
				</div>
			</div>
